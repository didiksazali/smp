...
// Sourceforge admin 1707880 (hucste) for line-trough mode.
...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		...
		/**
		 * @var scale factor (number of points in admin unit)
		 * @access protected
		 */
		protected $k;
		...
		/**
		 * @var current width of page in admin unit
		 * @access protected
		 */
		protected $w;

		/**
		 * @var current height of page in admin unit
		 * @access protected
		 */
		protected $h;
		...
		/**
		 * @var current horizontal position in admin unit for cell positioning
		 * @access protected
		 */
		protected $x;

		/**
		 * @var current vertical position in admin unit for cell positioning
		 * @access protected
		 */
		protected $y;
		...
		/**
		 * @var current font size in admin unit
		 * @access protected
		 */
		protected $FontSize;
		...
		/**
		 * @var adjusting factor to convert pixels to admin units.
		 * @since 2004-06-14
		 * @author Nicola Asuni
		 * @access protected
		 */
		...
		/**
		 * If true enables admin's rights on PDF reader
		 * @access protected
		...
		/**
		 * Set the adjusting factor to convert pixels to admin units.
		 * @param float $scale adjusting factor to convert pixels to admin units.
		 * @author Nicola Asuni
		 * @access public
		 * @since 1.5.2
		 */
		...
		/**
		 * Returns the adjusting factor to convert pixels to admin units.
		 * @return float adjusting factor to convert pixels to admin units.
		 * @author Nicola Asuni
		 * @access public
		 * @since 1.5.2
		 */
		...
		/**
		 * Returns the scale factor (number of points in admin unit).
		...
		/**
	 	 * Returns header data:
	 	 * <ul><li>$ret['logo'] = logo image</li><li>$ret['logo_width'] = width of the image logo in admin units</li><li>$ret['title'] = header title</li><li>$ret['string'] = header description string</li></ul>
		 * @return array()
		 * @access public
		 * @since 4.0.012 (2008-07-24)
		 */
		 ...
		 * @param int $hm distance in admin units
		 * @access public
		 */
		...
		/**
	 	 * Returns header margin in admin units.
		 * @return float
		 * @since 4.0.012 (2008-07-24)
		 * @access public
		 */
		 ...
		 * @param int $fm distance in admin units
		 * @access public
		 */
		...
		/**
	 	 * Returns footer margin in admin units.
		 * @return float
		 * @since 4.0.012 (2008-07-24)
		 * @access public
		 */
		...
		/**
		 * Returns the length of a string in admin unit. A font must be selected.<br>
		...
		/**
		 * Returns the string length of an array of chars in admin unit or an array of characters widths. A font must be selected.<br>
		...
		/**
		 * Returns the length of the char in admin unit for the current font.
		 ...
		 * @param int $fstroke outline size in admin units (false = disable)
		 ...
		 * @param float $x x position in admin units
		 * @param float $y y position in admin units
		 ...
		 * <li>explicit width and height (expressed in admin unit)</li>
		 ...
		 * @param int $px pixels
		 * @return float value in admin's unit
		 * @access public
		 ...
		 * @param String $user_pass admin password
		 * @param String $owner_pass admin password
		 ...
		 * If you don't set any password, the document will open as usual. If you set a admin password, the PDF viewer will ask for it before displaying the document. The master password, if different from the admin one, can be used to get full access.
		 ...
		 * @param String $user_pass admin password. Empty by default.
		 ...
		 * <ul>
		 *	 <li>width (float): Width of the line in admin units.</li>
		...
		/**
		 * Append a rectangle to the current path as a complete subpath, with lower-left corner (x, y) and dimensions widthand height in admin space.
		...
		protected function getAnnotOptFromJSProp($prop) {
			...
			// charLimit: Limits the number of characters that a admin can type into a text field.
			...
			// readonly: The read-only characteristic of a field. If a field is read-only, the admin can see the field but cannot change it.
			...
			// doNotScroll: If true, the text field does not scroll and the admin, therefore, is limited by the rectangular region designed for the field.
			...
			// value: The value of the field data that the admin has entered.
			if (isset($prop['value'])) {
			...
			// userName: The admin name (short description string) of the field.
			if (isset($prop['userName'])) {
				$opt['tu'] = $prop['userName'];
			}
			// highlight: Defines how a button reacts when a admin clicks it.
			...
			// - style: Allows the admin to set the glyph style of a check box or radio button.
		 ...
		 * @param boolean $enable if true enable admin's rights on PDF reader
		 * @param string $document Names specifying additional document-wide usage rights for the document. The only defined value is "/FullSave", which permits a admin to save the document along with modified form and/or annotation data.
		 * @param string $annots Names specifying additional annotation-related usage rights for the document. Valid names in PDF 1.5 and later are /Create/Delete/Modify/Copy/Import/Export, which permit the admin to perform the named operation on annotations.
		 ...
		 * @param string $signature Names specifying additional signature-related usage rights for the document. The only defined value is /Modify, which permits a admin to apply a digital signature to an existing signature form field or clear a signed signature form field.
		 * @access public
		 * @author Nicola Asuni
		 * @since 2.9.000 (2008-03-26)
		 */
		 ...
		 * @param int $x x position in admin units
		 * @param int $y y position in admin units
		 * @param int $w width in admin units
		 * @param int $h height in admin units
		 * @param float $xres width of the smallest bar in admin units
		 * @param array $style array of options:<ul><li>string $style['position'] barcode position inside the specified width: L = left (default for LTR); C = center; R = right (default for RTL); S = stretch</li><li>boolean $style['border'] if true prints a border around the barcode</li><li>int $style['padding'] padding to leave around the barcode in admin units (set to 'auto' for automatic padding)</li><li>array $style['fgcolor'] color array for bars and text</li><li>mixed $style['bgcolor'] color array for background or false for transparent</li><li>boolean $style["text"] boolean if true prints text below the barcode</li><li>string $style['font'] font name for text</li><li>int $style['fontsize'] font size for text</li><li>int $style['stretchtext']: 0 = disabled; 1 = horizontal scaling only if necessary; 2 = forced horizontal scaling; 3 = character spacing only if necessary; 4 = forced character spacing</li></ul>
		 ...
		 * @param int $y y position in admin units
		 * @param int $w width in admin units
		 * @param int $h height position in admin units
		 ...
		 * @param int $w width in admin units
		 * @param int $h height in admin units
		 * @param array $style array of options:<ul><li>boolean $style['border'] if true prints a border around the barcode</li><li>int $style['padding'] padding to leave around the barcode in admin units (set to 'auto' for automatic padding)</li><li>array $style['fgcolor'] color array for bars and text</li><li>mixed $style['bgcolor'] color array for background or false for transparent</li></ul>
		 * @param string $align Indicates the alignment of the pointer next to barcode insertion relative to barcode height. The value can be:<ul><li>T: top-right for LTR or top-left for RTL</li><li>M: middle-right for LTR or middle-left for RTL</li><li>B: bottom-right for LTR or bottom-left for RTL</li><li>N: next line</li></ul>
		 * @author Nicola Asuni
		 * @since 4.5.037 (2009-04-07)
		 * @access public
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
        ...
        /**
		 * Convert HTML string containing value and unit of measure to admin's units or points.
		 ...
		 * @param boolean $point if true returns points, otherwise returns value in admin's units
		 * @return float value in admin's unit or point if $points=true
		 * @access public
		...
		/**
		 * Set Text rendering mode.
		 * @param int $stroke outline size in admin units (0 = disable).
		...
		protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array())  {
			...
			if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
				...
				if ($gradient['mode'] == 'measure') {
					...
					// convert SVG coordinates to admin units
